...
class DataTransform:
    ...
    def _get_songs_dict(self, melted_album_df: pd.DataFrame) -> Dict[Tuple[Any, Any], List[str]]:
        album_songs_dict = {}
        for index, row in melted_album_df.iterrows():
            album = row['album_title']
            songs = self.mbz_client.get_songs(album=album,
                                              artist_id=row['id'])
            if songs:
    ...
    def _fix_album_df(self, album_df: pd.DataFrame) -> pd.DataFrame:
        ...
        melted_album_df = pd.melt(album_df,
                                  id_vars=['id'],
                                  var_name='album_num',
                                  value_name='album_title')
    ...
    @staticmethod
    def save_to_csv(songs_df: pd.DataFrame, artist_df: pd.DataFrame, albums_df: pd.DataFrame):
    ...
    def process(self):
        ...
        self.save_to_csv(songs_df=melted_df,
                         albums_df=melted_album_df,
                         artist_df=df)
        self.copy_csv_data()
